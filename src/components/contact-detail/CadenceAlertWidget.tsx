...
export const CadenceAlertWidget = React.memo(function CadenceAlertWidget() {
  ...
  if (loading) {
    return (
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Cadência</CardTitle></CardHeader>
            ...
            return (
              <div key={alert.cadence.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {alert.company?.name && (
                    <p className="text-[10px] text-muted-foreground truncate">{alert.company.name}</p>
                  )}
                </div>

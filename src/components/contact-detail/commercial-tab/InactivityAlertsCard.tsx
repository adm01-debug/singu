        ...
        {alerts?.map(alert => (
          <div key={alert.id} className={`p-2 rounded-md border border-border/50 ${alert.dismissed ? 'opacity-50' : ''}`}>
            <div className="flex items-center justify-between">
              ...
              <Badge className={`text-[9px] ml-2 ${severityColors[alert.severity || 'info'] || severityColors.info}`}>
                {alert.severity || 'info'}
              </Badge>
            </div>
            <div className="flex items-center gap-2 mt-1">
              {alert.days_inactive != null && (
                <span className="text-[10px] text-muted-foreground flex items-center gap-0.5">
                  <Clock className="h-2.5 w-2.5" />
                  {alert.days_inactive} dias inativo
                </span>
              )}
              {alert.last_interaction_at && (
                <span className="text-[10px] text-muted-foreground">
                  Última: {format(new Date(alert.last_interaction_at), 'dd/MM/yy')}
                </span>
              )}
            </div>
            {alert.description && (

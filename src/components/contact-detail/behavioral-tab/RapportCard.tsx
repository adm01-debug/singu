import { Handshake, Zap, TrendingUp, MessageSquare } from 'lucide-react';
...
export function RapportCard({ contactId }: Props) {
  const { data } = useRapportPoints(contactId);
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            {data.rapport_level && (
              <Badge variant="outline" className={cn('text-xs', LEVEL_COLORS[data.rapport_level] || '')}>
                {data.rapport_level}
              </Badge>
            )}
            {data.rapport_status && (
              <p className="text-xs text-muted-foreground mt-0.5">{data.rapport_status}</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          {data.pace && (
            <div className="rounded border p-1.5 text-center">
              ...
              <p className="font-medium text-foreground">{data.pace}</p>
            </div>
          )}
          {data.tone && (
            <div className="rounded border p-1.5 text-center">
              <p className="text-muted-foreground">Tom</p>
              <p className="font-medium text-foreground">{data.tone}</p>
            </div>
          )}
          {data.detail_preference && (
            <div className="rounded border p-1.5 text-center">
              <p className="text-muted-foreground">Detalhes</p>
              <p className="font-medium text-foreground">{data.detail_preference}</p>
            </div>
          )}
          {data.decision_style && (
            <div className="rounded border p-1.5 text-center">
              <p className="text-muted-foreground">Decisão</p>
              <p className="font-medium text-foreground">{data.decision_style}</p>
            </div>
          )}
        </div>
        ...
        {data.shared_interests && data.shared_interests.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {data.shared_interests.map((i, idx) => (
        ...
        {data.mirroring_tips && data.mirroring_tips.length > 0 && (
          <div>
            <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-1">
              <MessageSquare className="h-3 w-3" /> Dicas de Espelhamento
            </p>
            <ul className="space-y-0.5">
              {data.mirroring_tips.slice(0, 3).map((tip, i) => (
        ...
        {data.improvement_suggestions && data.improvement_suggestions.length > 0 && (
          <div>
            <p className="text-xs font-medium text-muted-foreground flex items-center gap-1 mb-1">
              <TrendingUp className="h-3 w-3" /> Melhorias Sugeridas
            </p>
            <ul className="space-y-0.5">
              {data.improvement_suggestions.slice(0, 3).map((s, i) => (

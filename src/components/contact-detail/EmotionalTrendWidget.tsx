...
const TREND_COLOR = { rising: 'text-success', stable: 'text-muted-foreground', declining: 'text-destructive' };
...
export const EmotionalTrendWidget = React.memo(function EmotionalTrendWidget({ contactId }: { contactId: string }) {
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-sm">Tendência Emocional</CardTitle></CardHeader>
      ...
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            {data.trend_status && (
              <Badge variant="outline" className="text-[10px]">{data.trend_status}</Badge>
            )}
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded-lg bg-success/10 p-2">
            <p className="text-sm font-bold text-success">{data.positive_interactions || 0}</p>
            <p className="text-[9px] text-muted-foreground">Positivas</p>
          </div>
          <div className="rounded-lg bg-muted p-2">
            <p className="text-sm font-bold text-muted-foreground">{data.neutral_interactions || 0}</p>
            <p className="text-[9px] text-muted-foreground">Neutras</p>
          </div>
          <div className="rounded-lg bg-destructive/10 p-2">
            <p className="text-sm font-bold text-destructive">{data.negative_interactions || 0}</p>
            <p className="text-[9px] text-muted-foreground">Negativas</p>
          </div>
        </div>
        ...
        {data.triggers_positive && data.triggers_positive.length > 0 && (
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap gap-1">
              {data.triggers_positive.slice(0, 4).map((t, i) => (
                <Badge key={i} variant="outline" className="text-[9px] border-success/30 text-success">{t}</Badge>
              ))}
            </div>
          </div>
        )}
        ...
        {data.recommended_actions && data.recommended_actions.length > 0 && (
          <div className="space-y-1">
            ...
            <ul className="space-y-0.5">
              {data.recommended_actions.slice(0, 2).map((a, i) => (

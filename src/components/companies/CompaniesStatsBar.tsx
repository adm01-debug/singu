...
import { Building2, Users, CheckCircle2, AlertTriangle, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        {stats.carriers > 0 && (
          <StatCard icon={TrendingUp} label="Transportadoras" value={stats.carriers} color="text-muted-foreground" />
        )}
        {stats.suppliers > 0 && (
          <StatCard icon={TrendingUp} label="Fornecedores" value={stats.suppliers} color="text-muted-foreground" />
        )}
      </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span className="text-[11px] font-semibold text-primary tabular-nums">
                {stats.conversionRate}% convertidos
              </span>
            </div>
            <div className="relative h-2 rounded-full bg-muted/30 overflow-hidden">
              <div
                className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-success to-success/70 transition-all duration-500"
                style={{ width: `${stats.conversionRate}%` }}
              />
            </div>
            <div className="flex items-center justify-between mt-1">
              <span className="text-[10px] text-success/70 tabular-nums">

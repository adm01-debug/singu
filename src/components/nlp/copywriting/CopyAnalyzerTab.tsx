...
interface CopyAnalyzerTabProps {
  analyzeText: string;
...
export default function CopyAnalyzerTab({
  analyzeText, setAnalyzeText, analysisResult, getScoreColor
}: CopyAnalyzerTabProps) {
  return (
          ...
          <div className="bg-muted/50 rounded-lg p-4">
            ...
            {analysisResult.triggerDensity.dominantTriggers.length > 0 && (
              <div className="flex flex-wrap gap-1 mb-2">
                ...
                {analysisResult.triggerDensity.dominantTriggers.map((t: string) => (
            ...
            {analysisResult.triggerDensity.missingTriggers.length > 0 && (
              <div className="flex flex-wrap gap-1">
                ...
                {analysisResult.triggerDensity.missingTriggers.slice(0, 4).map((t: string) => (
                ...
                <ul className="space-y-2">
                  {analysisResult.issues.map((issue: any, idx: number) => (
                    <li key={idx} className="text-sm">
                      <span className={cn("font-medium", issue.severity === 'high' ? 'text-destructive' : 'text-warning')}>
                        {issue.issue}
                      </span>

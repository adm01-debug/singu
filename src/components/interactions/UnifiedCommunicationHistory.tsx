...
const channelIcons: Record<string, typeof MessageSquare> = {
  whatsapp: MessageSquare, call: Phone, email: Mail, meeting: Users, note: Edit, social: Globe,
};
const channelColors: Record<string, string> = {
  whatsapp: 'bg-success/10 text-success', call: 'bg-info/10 text-info',
  email: 'bg-primary/10 text-primary', meeting: 'bg-warning/10 text-warning',
    ...
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm flex items-center gap-2">
          <MessageSquare className="h-4 w-4 text-primary" />Comunicação Unificada
          ...
          {history.map((item: InteractionHistoryItem) => {
            ...
            return (
              <div key={item.id} className="flex items-start gap-3 p-2.5 rounded-lg hover:bg-muted/30 transition-colors border border-border/30">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${colorClass}`}><Icon className="h-4 w-4" /></div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {item.direction && <span className="text-[9px] text-muted-foreground">{item.direction === 'inbound' ? '← Recebido' : '→ Enviado'}</span>}
                    {item.data_interacao && <span className="text-[9px] text-muted-foreground ml-auto">{format(new Date(item.data_interacao), "dd/MM/yy HH:mm", { locale: ptBR })}</span>}
                  </div>
                </div>
              </div>
            );

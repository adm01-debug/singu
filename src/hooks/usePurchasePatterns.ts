...
export function usePurchasePatterns() {
  ...
  const analyzePurchasePatterns = useCallback(async () => {
    ...
    try {
      ...
      type PurchaseRow = NonNullable<typeof purchases>[number];
      const purchasesByContact = new Map<string, PurchaseRow[]>();
      ...
      purchasesByContact.forEach((contactPurchases, contactId) => {
        ...
        contactPurchases.forEach((p) => {
          ...
          if (p.product_category) {
            categoryCounts.set(
              p.product_category,
              (categoryCounts.get(p.product_category) || 0) + 1,
            );
          }
        });
        ...
        const purchaseFrequency =
          averageCycleDays <= 30 ? 'high' : averageCycleDays <= 90 ? 'medium' : 'low';
      ...
      const categories: CategoryPattern[] = Array.from(categoryMap.entries()).map(
        ([category, data]) => ({
          category,
  ...
  const stats = useMemo(() => {
    ...
    const upcomingWeek = patterns.filter(
      (p) => !p.isOverdue && p.daysUntilPredictedPurchase <= 7,
    ).length;
